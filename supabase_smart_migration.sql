-- УМНАЯ МИГРАЦИЯ GUMROAD В SUPABASE
-- Создан автоматически: 2025-08-28 13:00:46 +0300
-- Проверяет существование объектов перед созданием

-- Устанавливаем режим для игнорирования несуществующих объектов
SET client_min_messages TO WARNING;

-- ==================================
-- 1. СОЗДАНИЕ SEQUENCES (ЕСЛИ НЕ СУЩЕСТВУЮТ)
-- ==================================
...
-- ==================================
-- 4. ALTER TABLE КОМАНДЫ (С ОБРАБОТКОЙ ОШИБОК)
-- ==================================
-- Alter 1/346
DO $$ 
BEGIN
...
WHEN duplicate_object THEN
    -- Объект уже существует, игнорируем
    NULL;
WHEN others THEN
...
-- ==================================
SELECT 'Создано таблиц:' as info, count(*) as count FROM information_schema.tables WHERE table_schema = 'public';
SELECT 'Создано индексов:' as info, count(*) as count FROM pg_indexes WHERE schemaname = 'public';
SELECT 'Создано sequences:' as info, count(*) as count FROM pg_sequences WHERE schemaname = 'public';

-- УМНАЯ МИГРАЦИЯ ЗАВЕРШЕНА
